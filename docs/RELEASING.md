...
3. Writes that version everywhere it lives (the manifest and both csproj files), and for a stable release turns the
...
The title is `XivMcp vX.Y.Z`, or `XivMcp vX.Y.Z-test.N (testing)`. The notes are
